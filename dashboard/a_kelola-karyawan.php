...
if (isset($_POST['add_data'])) {
	...
	$role = $connect->real_escape_string(trim(filter($_POST['role'])));
	...
	//Jika data username sudah ada didalam database, jika iya maka proses daftar gagal
		...
		$_SESSION['notification'] = array('alert' => 'error', 'title' => 'Gagal', 'message' => 'Username <strong>(' . $username . ')</strong> Sudah terdaftar.');
	}
	...
	//Apakah password1 dan password2 cocok? jika tidak maka proses daftar gagal
		...
		$_SESSION['notification'] = array('alert' => 'error', 'title' => 'Gagal', 'message' => 'Konfirmasi password tidak cocok, mohon ketik ulang password anda.');
	}
	...
	//Apabila semua "if" diatas terlewati maka..
		...
		//Jika Berhasil memasukan data kedalam database, maka proses daftar berhasil;
		if ($connect->query("INSERT INTO user (full_name, username, password, role) VALUES ('$fullName', '$username','$password_hash', '$role')") == true) {
						...
						<div class="card-body">
							<table id="example1" class="table table-bordered table-striped">
								...
								<tbody>
									...
									$getUsers = $connect->query("SELECT * FROM user WHERE username != '$session_username'");
									while ($dataUsers = $getUsers->fetch_assoc()) {
									?>
										<tr>
											<td><?= $no ?></td>
											<td><?= $dataUsers['full_name'] ?></td>
													...
													<div class="modal-dialog">
														<div class="modal-content">
															...
															<div class="modal-footer">
																<button type="button" class="btn btn-error" data-dismiss="modal"><i class="fas fa-times btn-xs"></i> Batal</button>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST">
					<div class="input-group mb-3">
						<input type="text" name="fullName" class="form-control" value="<?= empty($fullName) ? "" : $fullName; ?>" placeholder="Nama Lengkap">
						<div class="input-group-append">
							<div class="input-group-text">
								<span class="fa fa-id-card"></span>
							</div>
