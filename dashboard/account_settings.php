		...
		if ($dataUser->num_rows > 0) {
			...
			if (strlen($new_password) < 4) {
				...
				if ($connect->query("UPDATE user SET password ='$password_hash' WHERE id='$idUser'")) {
					...
					$_SESSION['notification'] = array('alert' => 'error', 'title' => 'Gagal', 'message' => 'Perubahan gagal tersimpan, silahkan coba lagi! Error : ' . mysqli_error($connect));
				}
			} else {
				$_SESSION['notification'] = array('alert' => 'error', 'title' => 'Gagal', 'message' => 'Password saat ini kamu salah.');
			}
		}
	} else {
